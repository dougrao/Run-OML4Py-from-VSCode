...
#### Construindo um experimento com OML4Py

Após ter estabelecido e testado a conexão com o `Server OML4Py` podemos construir modelos de *Machine Learning* e utilizar todos os recursos do processamento *in-database*. No exemplo a seguir, será construído um modelo de Detecção de Anomalias utilizando SVM e tabelas de exemplo já presentes no `Autonomous Database`. No mesmo arquivo *Python* criado na etapa anterior inclua as linhas abaixo
...
Com isso temos um modelo SVM com a capacidade de identificação de anomalias. Neste exemplo são exibidos os 5 casos mais anômalos e o modelo gerado é armazenado no *datastore* do `Autonomous Database`.

Para conhecer mais sobre o `Oracle Machine Learning` e explorar outros algoritmos acesse este [link](https://docs.oracle.com/en/database/oracle/machine-learning/oml4py/index.html).
